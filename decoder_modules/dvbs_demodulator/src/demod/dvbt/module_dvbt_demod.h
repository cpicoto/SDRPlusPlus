...
namespace dsp::dvbt {
    ...
    private:
        ...
        bool decodeTPS(uint8_t* tps_bits, float quality);
        void demodulateData(complex_t* ofdm_symbols, uint8_t* output);
        void updateTPSLock();
        void updateOFDMParameters();  // Dynamic mode switching
        void calculateSNR(complex_t* pilots);
        void estimateFrequencyOffset(complex_t* pilots);
        void applyFrequencyCorrection(complex_t* samples, int count);
        ...
        void processRSPacket(uint8_t* packet, bool debug);
        bool findTSSync(uint8_t* data, int length, int& sync_pos);
        int findSyncByteAlignment(uint8_t* data, int length);
        
        // DVB-T Inner Processing Chain
        void bitInnerDeinterleave(uint8_t* input, uint8_t* output, int length);
        int bitInnerDeinterleaverH(int e, int w);  // H function from gr-dvbt
        void depunctureGNURadio(const std::vector<uint8_t>& input, std::vector<uint8_t>& output, int code_rate);
        int viterbiDecode(uint8_t* input, uint8_t* output, int length);
        void energyDescramble(uint8_t* input, uint8_t* output, int length);
        ...
        // Current mode tracking for dynamic switching
        enum Mode { MODE_2K, MODE_4K, MODE_8K };
        Mode current_mode = MODE_8K;  // Default to 8K mode
        ...
        // DVB-T Inner Processing State
        struct BitInnerDeinterleaver {
            static const int BLOCK_SIZE = 126;
            int permutation[BLOCK_SIZE * 2];  // For QPSK
            bool initialized = false;
        } bit_deinterleaver;
        
        struct ViterbiDecoder {
            correct_convolutional* decoder = nullptr;
            bool initialized = false;
        } viterbi_decoder;
        
        struct EnergyDescrambler {
            int prbs_register = 0xa9;  // Initial PRBS state
            bool initialized = false;
        } energy_descrambler;
